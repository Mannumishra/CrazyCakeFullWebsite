      ...
      {categoryData.map((category) => {
        ...
        return (
          <div key={categoryId} className="featured-menu">
            <h2 className="featured-menu-title text-uppercase">{category.mainCategoryName}</h2>
              ...
              {visibleProducts.map((product) => (
                <div key={product._id} className="product-card">
                  ...
                  <p className="d-flex gap-2 align-items-center">
                    <span className="product-main-price">{product.Variant[0].finalPrice.toFixed(2)}
                    </span>
            ...
            <div className="pagination-controls">
              {hasPrevPage && (
                <button className="prev-btn" onClick={() => handlePrevPage(categoryId)}>
                  Previous
                </button>
              )}
              {hasNextPage && (
                <button className="next-btn" onClick={() => handleNextPage(categoryId)}>
                  Next
                </button>
              )}
            </div>
