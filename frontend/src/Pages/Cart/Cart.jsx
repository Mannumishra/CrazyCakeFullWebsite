...
const Cart = () => {
  ...
  const handleDecrement = (id, weight) => {
    setCartItems((prevItems) => {
      const updatedItems = prevItems.map((item) =>
        getUniqueId(item.id, item.weight) === getUniqueId(id, weight) && item.quantity > 1
                ...
                <div key={getUniqueId(item.id, item.weight)} className="cart-item">
                  ...
                  <div className="item-details">
                    ...
                    <div className="quantity-controls">
                      <button onClick={() => handleDecrement(item.id, item.weight)}>-</button>
                      <span>{item.quantity || 1}</span>
                      <button onClick={() => handleIncrement(item.id, item.weight)}>+</button>
                    </div>
                  </div>
                  <button
